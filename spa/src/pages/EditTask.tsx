    ...
    const fetchData = async () => {
      try {
        const [catsRes, taskRes] = await Promise.all([
          ...
          apiClient.get<Task>(`/tasks/my`) // We filter manually or fetch by ID if allowed
        ]);
        
        setCategories(catsRes.data);
        
        // Find the specific task in 'my tasks' or fetch by ID
        // For security and simplicity, we reuse the task data if we can
        const task = taskRes.data; // Wait, /tasks/my returns a list
        
        // Let's fetch by ID instead if we want to be sure
        const taskDetailRes = await apiClient.get<Task>(`/tasks/${id}`);
